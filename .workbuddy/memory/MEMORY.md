...
- **Windows 桌面端（`Sources/BrewPingwinDesktop`，Tauri 2 + axum 0.8）**：
  ...
  - **⚠️ cargo test 的测试二进制绝不能链入 tauri GUI 类型**（2026-09-12 踩到 STATUS_ENTRYPOINT_NOT_FOUND / 0xc0000139）：AppState 一旦含 `tauri::AppHandle`，测试 exe 会链入整个 GUI 窗口栈（user32/uxtheme/comctl32…），无应用 manifest 的测试进程加载即崩。**解法 = `EventSink` 模式**（http_server.rs）：`pub type EventSink = Arc<dyn Fn(&str, serde_json::Value) + Send + Sync>`，http_server/command_runner 零 tauri 依赖，lib.rs 运行时注入闭包发事件。改 AppState 加字段前先想这个。
  - **多对话管理已落地**（方案 `docs/BrewPing-Windows端多对话管理实现方案.md`，P1-P4 全部完成）：`conversation_store.rs`（index.json + conv_<id>.json 两层分离，落盘 `~/.brewping/conversations/`）+ `command_runner.rs`（唯一执行路径+唯一命令状态写入点）+ `conversation_api.rs`（6 端点）+ lib.rs 7 个 Tauri 命令；前端 App.tsx 侧栏三区（置顶/对话/已归档）+ 草稿态（新对话零 API，首条消息物化）+ per-conversation drafts。iOS 兼容承诺：`/api/status.session` = active 对话渲染；HTTP `/api/status` 无 `activeConversationId`（只有 Tauri `get_status` 有），属正常。
...
- **macOS 桌面端（Sources/，SwiftPM MenuBarExtra）多对话契约已对齐 Windows（2026-09-12）**：`Sources/App/ConversationStore.swift`（index.json + conv_<id>.json，键与 serde camelCase 逐字对齐）+ HTTPAPI 6 条 /api/conversations 路由 + `/api/message` conversationId 三层回落 + ApprovalGate 按对话档位（`check(text:mode:conversationID:)`）+ CommandContext（cwd/模型仅 headless 型 Agent 生效；opencode 会话型进程 cwd/模型启动时固定）。UI 仍是菜单栏 popover + 终端窗口，无 i18n；无 XCTest。⚠️ Mac 上改完必须 `swift build && swift test`。
