...
> **状态更新（2026-09-21）**：本文件是当时的一次性审计记录，其中的建议已部分落地。
> 具体地，`relay-server/` 已按本文件建议的思路**移入 `experimental/relay-server/`**
> （未删除，保持参考价值），并在 README / SECURITY / PROJECT_STATUS 中明确它不属于
> 已发布产品、不在产品安全边界内。阅读时请以当前目录结构为准。
